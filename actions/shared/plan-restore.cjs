...
var METRO_TRANSFORM_DIRNAME = "metro";
var METRO_FILE_MAP_DIRNAME = "metro-file-map";
var createHarnessCachePaths = (projectRoot) => {
  const root = import_node_path7.default.join(projectRoot, HARNESS_DIRNAME, CACHE_DIRNAME);
  return {
    root,
    metroTransform: import_node_path7.default.join(root, METRO_TRANSFORM_DIRNAME),
    metroFileMap: import_node_path7.default.join(root, METRO_FILE_MAP_DIRNAME)
  };
};
var getDomainDirectories = (domain, paths) => {
  switch (domain) {
    case "metro":
...
var KEYS_FILENAME = "keys.json";
var HASH_LENGTH = 16;
var ALL_DOMAINS = ["metro"];
var isAccretiveDomain = (domain) => {
  switch (domain) {
    case "metro":
      return true;
    default:
...
var buildStaticHashKey = (domain, inputs) => {
  const hash = hashSortedEntries(Object.entries(inputs.staticInputs));
  return `harness-${domain}-${inputs.os}-${hash}`;
};
var buildContentHash = (entries) => hashSortedEntries(entries.map((entry) => [entry.path, String(entry.sizeBytes)]));
var walkDirectory = (root, directory, entries) => {
  let dirents;
  try {
    dirents = import_node_fs6.default.readdirSync(directory, { withFileTypes: true });
  } catch {
    return;
  }
  for (const dirent of dirents) {
    const fullPath = import_node_path8.default.join(directory, dirent.name);
    if (dirent.isDirectory()) {
      walkDirectory(root, fullPath, entries);
    } else if (dirent.isFile()) {
      const stat = import_node_fs6.default.statSync(fullPath);
      entries.push({
        path: import_node_path8.default.relative(root, fullPath),
        sizeBytes: stat.size,
...
var snapshot = (paths) => {
  ...
  for (const domain of ALL_DOMAINS) {
    try {
      ...
      cacheLogger.warn(`Failed to snapshot cache domain "${domain}". Treating as empty.`, error);
      result[domain] = [];
    }
  }
  return result;
};
var diffSnapshots = (before, after) => {
...
var shouldSaveForPolicy = (policy, delta) => {
...
var planSave = (paths, before, policy, inputs) => {
  ...
  for (const domain of ALL_DOMAINS) {
    ...
    domains[domain] = {
      ...
      saveKey: accretive ? `${staticHashKey}-${buildContentHash(afterEntries)}` : staticHashKey,
      delta
    };
  }
  return { version: 1, domains };
};
var writeKeysFile = (paths, plan) => {
  const filePath = import_node_path8.default.join(paths.root, KEYS_FILENAME);
  try {
    import_node_fs6.default.mkdirSync(paths.root, { recursive: true });
    import_node_fs6.default.writeFileSync(filePath, JSON.stringify(plan, null, 2));
  } catch (error) {
    cacheLogger.warn(`Failed to write cache keys file "${filePath}".`, error);
  }
};

// ../cache/dist/warmth.js
var import_node_fs7 = __toESM(require("fs"), 1);
var cacheLogger2 = logger.child("cache");
var isDomainWarm = (domain, paths) => {
  const directory = getWarmthDirectory(domain, paths);
  if (!directory) {
    return false;
  }
  try {
    const stat = import_node_fs7.default.statSync(directory);
    if (!stat.isDirectory()) {
      return false;
    }
    return import_node_fs7.default.readdirSync(directory).length > 0;
  } catch (error) {
    if (isMissingPathError(error)) {
      return false;
    }
    cacheLogger2.warn(`Failed to check cache warmth for "${domain}" at "${directory}". Treating as cold.`, error);
    return false;
  }
};
var getWarmthDirectory = (domain, paths) => {
  switch (domain) {
    case "metro":
...
var hashKeyFiles = (matches) => {
  ...
  for (const match of matches.slice().sort((a, b) => a.relativePath < b.relativePath ? -1 : 1)) {
...
var createHarnessCache = (options) => {
  ...
  return {
    ...
    ensureDomainDirectories: (domain) => {
      for (const directory of getDomainDirectories(domain, paths)) {
        try {
          import_node_fs9.default.mkdirSync(directory, { recursive: true });
        } catch (error) {
          cacheLogger4.warn(`Failed to create cache directory "${directory}". Continuing with a cold cache.`, error);
        }
      }
    },
    planRestore: (inputs) => planRestore(paths, inputs),
    snapshot: () => snapshot(paths),
    planSave: (before, policy, inputs) => planSave(paths, before, policy, inputs),
...
var importUp = async (dir, name) => {
  const filePath = import_node_path10.default.join(dir, name);
  for (const ext of extensions) {
    const filePathWithExt = `${filePath}${ext}`;
    if (import_node_fs10.default.existsSync(filePathWithExt)) {
      let rawConfig;
      try {
        if (ext === ".mjs") {
      ...
      try {
        ...
        if (error instanceof ZodError) {
          const validationErrors = error.errors.map((err) => {
            const path12 = err.path.length > 0 ? ` at "${err.path.join(".")}"` : "";
            return `${err.message}${path12}`;
          });
  ...
  const parentDir = import_node_path10.default.dirname(dir);
  if (parentDir === dir) {
...
var FALLBACK_BUNDLER_METRO_VERSION = "unknown";
var resolveRepoRoot = (projectRoot) => {
  let dir = projectRoot;
  while (true) {
    if (import_node_fs11.default.existsSync(import_node_path11.default.join(dir, ".git"))) {
      return dir;
    }
...
var resolveBundlerMetroVersion = (projectRoot) => {
  try {
    ...
    console.warn(
      `Failed to resolve @react-native-harness/bundler-metro's version from "${projectRoot}". Falling back to "${FALLBACK_BUNDLER_METRO_VERSION}".`,
      error
    );
...
var run = async () => {
  try {
    ...
    const projectRoot = projectRootInput ? import_node_path12.default.resolve(projectRootInput) : process.cwd();
    ...
    const output = `metroRestoreKey=${metroPlan?.restoreKey ?? ""}
metroRestorePrefixes=${JSON.stringify(
      metroPlan?.restorePrefixes ?? []
    )}
`;
    import_node_fs12.default.appendFileSync(githubOutput, output);
  } catch (error) {
