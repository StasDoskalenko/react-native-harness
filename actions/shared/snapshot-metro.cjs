      ...
      try {
        ...
        if (error instanceof ZodError) {
          const validationErrors = error.errors.map((err) => {
            const path11 = err.path.length > 0 ? ` at "${err.path.join(".")}"` : "";
            return `${err.message}${path11}`;
          });
...
var run = async () => {
  try {
    ...
    const projectRoot = projectRootInput ? import_node_path11.default.resolve(projectRootInput) : process.cwd();
